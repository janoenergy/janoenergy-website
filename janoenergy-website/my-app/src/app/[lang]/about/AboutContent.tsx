...
// 时间轴项目组件
function TimelineItem({ item, index, isLeft, lang }: { item: typeof timelineData[0]; index: number; isLeft: boolean; lang: 'zh' | 'en' }) {
  const { ref, isVisible } = useScrollAnimation();

  return (
    <div ref={ref} className={`flex items-center ${isLeft ? 'flex-row' : 'flex-row-reverse'}`}>
      <div 
        className={`w-5/12 ${isLeft ? 'text-right pr-4 md:pr-8' : 'text-left pl-4 md:pl-8'} transition-all duration-700 ${isVisible ? 'opacity-100 translate-x-0' : `opacity-0 ${isLeft ? '-translate-x-10' : 'translate-x-10'}`}`} 
        style={{ transitionDelay: `${index * 100}ms` }}
      >
        <div className="bg-white rounded-xl p-4 md:p-6 shadow-sm hover:shadow-lg transition-shadow">
          <h3 className="text-2xl md:text-3xl font-bold text-emerald-600 mb-2">{item.year}</h3>
          <h4 className="text-lg md:text-xl font-semibold text-gray-900 mb-2">{lang === 'zh' ? item.title : item.titleEn}</h4>
          <p className="text-sm md:text-base text-gray-600">{lang === 'zh' ? item.desc : item.descEn}</p>
        </div>
      </div>
      <div className="w-2/12 flex justify-center relative">
        <div 
          className={`w-4 h-4 bg-emerald-500 rounded-full border-4 border-white shadow-lg z-10 transition-all duration-500 ${isVisible ? 'scale-100' : 'scale-0'}`} 
          ...
          <FadeIn>
            <div>
              ...
              <div className="grid grid-cols-3 gap-4 md:gap-6 mt-8">
                <div className="text-center p-3 md:p-4 bg-white rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all">
        ...
        <div className="mb-16 md:mb-24">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {certificates.map((cert, idx) => (
              <FadeIn key={cert.name} delay={idx * 100}>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {valuesData.map((value, idx) => (
              <FadeIn key={value.title} delay={idx * 100}>
                <div className="bg-white rounded-xl p-6 md:p-8 text-center shadow-sm hover:shadow-lg transition-all h-full">
