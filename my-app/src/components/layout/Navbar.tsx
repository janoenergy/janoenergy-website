        ...
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-6">
            {navItems.map((item, index) => (
              <motion.a 
                ...
                whileHover={{ y: -2 }}
                className={`relative ${styles.navText} ${styles.navTextHover} font-medium transition-colors py-2 group`}
              >
                {item.label}
                <motion.span 
          ...
          <div className="md:hidden flex items-center gap-2">
            ...
            <div className="relative">
              <button 
                onClick={() => setLangMenuOpen(!langMenuOpen)}
                className={`flex items-center gap-1 px-2 py-1 text-sm font-medium border rounded-lg ${styles.navText} ${styles.border}`}
              >
                ...
                >
                  {languages.map((l) => (
                    <button
                      key={l.code}
                      onClick={() => switchLang(l.code)}
                      className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-emerald-500/10 ${
                        l.code === lang ? 'text-emerald-500 bg-emerald-500/10' : styles.navText
                      }`}
                    >
          ...
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navItems.map((item, index) => (
              <motion.a 
                ...
                className={`block px-3 py-2 ${styles.navText} ${styles.navTextHover} hover:bg-emerald-500/10 rounded-md transition-colors`} 
